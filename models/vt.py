...
class ViTResNet(nn.Module):
    def __init__(self, block, num_blocks, batch_size=64, num_classes=10, dim=128, num_tokens=8, mlp_dim=256, heads=8, depth=6,
        ...
        # Tokenization
        self.token_wA = nn.Parameter(torch.empty(batch_size, self.L, 64),
                                     requires_grad=True)  # Tokenization parameters
        torch.nn.init.xavier_uniform_(self.token_wA)
        self.token_wV = nn.Parameter(torch.empty(batch_size, 64, self.cT),
                                     requires_grad=True)  # Tokenization parameters
...
def VT(blocks, batch_size):
    return ViTResNet(BasicBlock, blocks, batch_size=batch_size)
